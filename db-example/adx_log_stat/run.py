...
# 解析日志，返回字段 字典类型
def build_new_dict_pla_style(date, hour, pla, style, bidf, tobid, wbid, dspWin):
    return {
        pla: {
            style: {
                'date': date,
...
def build_new_dict_style(date, hour, pla, style, bidf, tobid, wbid, dspWin):
    return {
        style: {
            'date': date,
            'hour': hour,
            ...
            if "logt=" in line:
                ...
                if pla in dict:
                    ...
                    if style in dict_pla:
                        ...
                        # create new style
                        dict_style = build_new_dict_style(date, hour, pla, style, bidf, tobid, wbid, dspWin)
                        dict_pla.update(dict_style)

                else:
                    # 下游id 不在dict中
                    dict_pla_style = build_new_dict_pla_style(date, hour, pla, style, bidf, tobid, wbid, dspWin)
...
def parse_sspinvalid_log():
    file_ = file_path + file_name + "." + last_hour2()
    dict_log = {}
    try:
        f = open(file_, 'r')
        # 按照pla的维度进行聚合
